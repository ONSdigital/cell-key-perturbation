...
The columns needed for an export are the variables which you have tabulated by 
(var1, var5, var8 in this example), plus the `ckey`, `pcv`, `pvalue` and `count` 
column. This is to allow the output checkers to verify that perturbation has 
been applied.
...
Your file name should end “.csv”. If you only specify a file name in the path it will save to your main directory. Setting ‘index’ to ‘False’ here stops it from adding a new ‘index’ column with the row number. This argument can be removed if you do want to create an index column.
